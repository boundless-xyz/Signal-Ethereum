...
impl Buf {
    ...
    fn raw_pivot(&self) -> u64 {
        let digest = sha2::Sha256::digest(&self.0[0..PIVOT_VIEW_SIZE]);
    ...
    fn mix_in_position(&mut self, position: u64) {
    ...
    fn hash(&self) -> FixedBytes<32> {
        sha2::Sha256::digest(self.0).as_slice().try_into().unwrap()
    }
}
...
pub fn shuffle_list(
    ...
    loop {
        ...
        let pivot = buf.raw_pivot() % list_size;
        let mirror = (pivot + 1) >> 1;
        buf.mix_in_position(pivot >> 8);
        let mut source = buf.hash();
        let mut byte_v: u8 = source[((pivot & 0xff) >> 3) as usize];
        ...
        for i in 0..mirror {
            ...
            if bit_v == 1 {
                input.swap(i.try_into().unwrap(), j.try_into().unwrap());
            }
        }
        ...
        let mut source = buf.hash();
        let mut byte_v: u8 = source[((end & 0xff) >> 3) as usize];

        for (loop_iter, i) in ((pivot + 1)..mirror).enumerate() {
            let j = end - loop_iter as u64;
